...
NrSlSciF2Header::PreSerialize(Buffer::Iterator& i) const
{
    ...
    scif2 = (m_srcId & 0xFFFF) | (scif2 << 16);  // 16 bits for srcId
    scif2 = (m_dstId & 0xFFFF) | (scif2 << 16);
    scif2 = (m_harqFbIndicator & 0x1) | (scif2 << 1);

    // Write 5 bytes (40 bits used)
    i.WriteU8((scif2 >> 32) & 0xFF);
    i.WriteHtonU32(scif2 & 0xFFFFFFFF);
}

uint32_t
NrSlSciF2Header::PreDeserialize(Buffer::Iterator& i)
{
    // Read 5 bytes (40 bits used)
    uint64_t scif2 = i.ReadU8();
    scif2 = (scif2 << 32) | i.ReadNtohU32();

    m_harqId = (scif2 >> 36) & 0xF;
    m_ndi = (scif2 >> 35) & 0x1;
    m_rv = (scif2 >> 33) & 0x3;
    m_srcId = (scif2 >> 17) & 0xFFFF;  // 16 bits for srcId
