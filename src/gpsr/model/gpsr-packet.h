...
/**
 * \brief Perimeter hop entry for path history (NS-2 style)
 */
struct PeriHop
{
    uint32_t ip;  ///< Node IP (as uint32_t)
    double x;     ///< X position
    double y;     ///< Y position
    double z;     ///< Z position (NS-2 compatibility)
};

/// Maximum hops in perimeter path (to limit header size)
static const uint8_t MAX_PERI_HOPS = 32;
...
class PositionHeader : public Header
{
  public:
    ...
    // Lf edge endpoints (NS-2 periptip)
    void SetLfEdge(uint32_t from, uint32_t to)
    {
        m_lfEdgeFrom = from;
        m_lfEdgeTo = to;
    }

    uint32_t GetLfEdgeFrom() const { return m_lfEdgeFrom; }
    uint32_t GetLfEdgeTo() const { return m_lfEdgeTo; }

    // === Hop History Methods (NS-2 style) ===
    
    /**
     * \brief Add a hop to the perimeter path history
     * \return true if added, false if MAX_PERI_HOPS reached
     */
    bool AddHop(uint32_t ip, double x, double y, double z = 0.0)
    {
        if (m_nhops >= MAX_PERI_HOPS)
            return false;
        m_hops[m_nhops].ip = ip;
        m_hops[m_nhops].x = x;
        m_hops[m_nhops].y = y;
        m_hops[m_nhops].z = z;
        m_nhops++;
    ...
    /**
     * \brief Get a hop from the path history
     */
    PeriHop GetHop(uint8_t index) const
    {
        if (index < m_nhops)
            return m_hops[index];
        return PeriHop{0, 0.0, 0.0, 0.0};
    }
     ...
     * Note: Uses directed matching (NS-2 style), not undirected
     */
    int FindEdge(uint32_t ip1, uint32_t ip2) const
    {
        for (int i = 0; i < m_nhops - 1; i++)
        {
            // Directed match only: ip1 followed by ip2
            if (m_hops[i].ip == ip1 && m_hops[i+1].ip == ip2)
            {
                return i;
            }
        }
  ...
  private:
    ...
    uint32_t m_lfEdgeFrom = 0;  // Lf edge source (NS-2 periptip[1])
    uint32_t m_lfEdgeTo = 0;    // Lf edge target (NS-2 periptip[0])
